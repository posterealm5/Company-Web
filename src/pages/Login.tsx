...
export default function Login() {
  ...
  }, [success, mode, navigate, triggerNotification]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (loadingAction || googleLoading || forgotCooldown !== null) return;
    ...
    try {
      if (mode === 'login') {
        ...
        // Let the redirect happen via useEffect with 600ms buffer (navigation triggered after auth state update)
      } else if (mode === 'signup') {
  ...
  const handleGoogleSignIn = async () => {
    if (loadingAction || googleLoading) return;
          ...
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button
                ...
                className="w-full bg-brand-red text-white font-bold uppercase tracking-wider py-4 hover:bg-red-700 transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed cursor-pointer active:scale-95 comic-border border-white"
              >
                {loadingAction ? (
